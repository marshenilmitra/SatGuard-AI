...
A production‑ready prototype that monitors spacecraft health using real ESA OPS‑SAT CubeSat telemetry, detects anomalies with an LSTM‑Autoencoder, and generates plain‑English diagnostic reports using a local RAG pipeline + Llama 3—all offline on a laptop.
...
🧠 System Architecture
https://images/rag_pipeline.png

The pipeline consists of four layers:

Data Ingestion & Preprocessing – CSV telemetry → pivot → fill → scale → windowing.

Anomaly Detection – LSTM‑Autoencoder computes reconstruction error; above‑threshold windows are anomalies.

Explainability & Diagnostics – RAG retrieves fault guidelines and LLM writes a report.

Presentation & Deployment – Streamlit dashboard, Docker container, Kafka POC.
